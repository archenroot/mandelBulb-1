...
inline vec3 getColour(const pixelData &pixData, const RenderParams &render_params, const vec3 &from, const vec3  &direction)
{
  ...
  if (pixData.escaped == false){
    ...
    VEC(hitColor,backColor.x, backColor.y,backColor.z);
 }
 return hitColor;
}
