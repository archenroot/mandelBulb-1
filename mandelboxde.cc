...
double MandelBoxDE(const vec3 &p0, const MandelBoxParams &params, double c1, double c2)
{
  ...
  while (i< params.num_iter && r2 < escape)
	...
	{
	  //p = p*(rFixed2rMin2);
	  VEC(p, p.x*rFixed2rMin2, p.y*rFixed2rMin2, p.z*rFixed2rMin2);
      ...
      if ( r2<rFixed2) 
	{
	  ...
	  //p = p*(rFixed2/r2);;
	  VEC(p, p.x*rFixed2/r2, p.y*rFixed2/r2, p.z*rFixed2/r2);
	  dfactor *= t;
	}
      ...
      //p = p*params.scale+p0;
	  	VEC(p, p.x*params.scale+p0.x, p.y*params.scale+p0.y, p.z*params.scale+p0.z);
      i++;
    }
  double Mag;
  MAGNITUDE(Mag, p);
